...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conectar->prepare("SELECT id, nome, senha, fkPerfil FROM usuarios WHERE email = ?");
    if ($stmt === false) {
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($senha, $usuario['senha'])) {
            ...
            $_SESSION['fkPerfil'] = $usuario['fkPerfil']; // Adicionar o perfil à sessão

            echo "<h1>Login realizado com sucesso. Bem-vindo!</h1>" . "<p>você será redirecionado em 3 segundos</p>" ;
        } else {
